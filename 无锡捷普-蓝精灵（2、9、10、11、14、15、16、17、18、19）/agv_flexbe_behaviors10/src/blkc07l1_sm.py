...
class BLKC07l1SM(Behavior):
	...
	def __init__(self):
		super(BLKC07l1SM, self).__init__()
		self.name = 'BLKC07l1'
	...
	def create(self):
		...
		with _state_machine:
			...
			# x:77 y:297
			OperatableStateMachine.add('blkc07Dwon',
										Navigation(position_x=38.305, position_y=131.705, position_z=0, orientation_x=0, orientation_y=0, orientation_z=0.7071, orientation_w=0.7071, frame_id='map'),
			...
			# x:512 y:601
			OperatableStateMachine.add('blkc07play',
										AlertPlay(sound_id=66, mode="loop", wait_time=5, single_time=3),
										transitions={'done': 'wait30s'},
										autonomy={'done': Autonomy.Off})

			# x:902 y:605
			OperatableStateMachine.add('blkc07stop',
										AlertLoopStop(),
			...
			OperatableStateMachine.add('blkc07up',
										Navigation(position_x=29.405, position_y=131.698, position_z=0, orientation_x=0, orientation_y=0, orientation_z=1, orientation_w=0, frame_id='map'),
			...
			OperatableStateMachine.add('do_forward_60',
										...
										transitions={'succeeded': 'blkc07play', 'failed': 'log5'},
			...
			OperatableStateMachine.add('lift_load',
										...
										transitions={'finished': 'blkc07Dwon'},
										autonomy={'finished': Autonomy.Inherit})
			...
			OperatableStateMachine.add('log6',
										LogState(text="blkc07up canceled", severity=Logger.REPORT_HINT),
										transitions={'done': 'blkc07up'},
										autonomy={'done': Autonomy.Off})
