...
class BLKC03l1SM(Behavior):
	...
	def __init__(self):
		super(BLKC03l1SM, self).__init__()
		self.name = 'BLKC03l1'
	...
	def create(self):
		...
		with _state_machine:
			...
			OperatableStateMachine.add('blkc03up',
										SiteNavigation(site_name=blkc03up),
			...
			OperatableStateMachine.add('do_forward_60',
										...
										transitions={'succeeded': 'playblkc03l1', 'failed': 'log5'},
										autonomy={'succeeded': Autonomy.Off, 'failed': Autonomy.Off})
			...
			OperatableStateMachine.add('lift_load',
										...
										transitions={'finished': 'blkc03down'},
										autonomy={'finished': Autonomy.Inherit})
			...
			OperatableStateMachine.add('log3',
										LogState(text="blkc03down canceled", severity=Logger.REPORT_HINT),
			...
			# x:513 y:600
			OperatableStateMachine.add('playblkc03l1',
										AlertPlay(sound_id=63, mode="loop", wait_time=3, single_time=3),
			...
			# x:898 y:597
			OperatableStateMachine.add('stopblkc03l1',
										AlertLoopStop(),
										transitions={'done': 'blkc03up'},
										autonomy={'done': Autonomy.Off})
			...
			OperatableStateMachine.add('wait30s',
										...
										transitions={'done': 'stopblkc03l1'},
										autonomy={'done': Autonomy.Off})

			# x:76 y:299
			OperatableStateMachine.add('blkc03down',
										SiteNavigation(site_name=blkc03down),
