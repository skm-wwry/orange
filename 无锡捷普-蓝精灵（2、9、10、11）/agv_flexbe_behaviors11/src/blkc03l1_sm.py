...
class BLKC03l1SM(Behavior):
	...
	def create(self):
		...
		with _state_machine:
			...
			# x:515 y:604
			OperatableStateMachine.add('blkc03play',
										AlertPlay(sound_id=63, mode="loop", wait_time=5, single_time=3),
										transitions={'done': 'wait30s'},
										autonomy={'done': Autonomy.Off})

			# x:899 y:605
			OperatableStateMachine.add('blkc03stop',
										AlertLoopStop(),
			...
			OperatableStateMachine.add('do_forward_60',
										...
										transitions={'succeeded': 'blkc03play', 'failed': 'log5'},
			...
			OperatableStateMachine.add('log6',
										LogState(text="blkc03up canceled", severity=Logger.REPORT_HINT),
										transitions={'done': 'blkc03up'},
										autonomy={'done': Autonomy.Off})
